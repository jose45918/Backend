...
async function editarUsuario(id, data) {
    const usuarioExistente = await usuariosBD.doc(id).get();
    if (!usuarioExistente.exists) {
        return false;
    }
    const usuarioActualizado = {
        nombre: data.nombre || usuarioExistente.data().nombre,
        usuario: data.usuario || usuarioExistente.data().usuario,
        password: data.password ? encriptarPassword(data.password).hash : usuarioExistente.data().password,
        salt: data.password ? encriptarPassword(data.password).salt : usuarioExistente.data().salt,
        tipoUsuario: data.tipoUsuario || usuarioExistente.data().tipoUsuario
    };
